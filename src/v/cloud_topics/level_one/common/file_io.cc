...
file_io::read_object(
  object_extent extent, ss::abort_source* as, cloud_io::group_id gid) {
    [[maybe_unused]] auto holder = _gate.hold();
    static constexpr auto timeout = 10s;
    static constexpr auto backoff = 100ms;
    retry_chain_node root(*as, ss::lowres_clock::now() + timeout, backoff);
    lazy_abort_source las{[as] {
    ...
    while (true) {
        ...
        auto cleanup = ss::defer([this, cache_key, &failure_errc]() {
            ...
            // mutated the map.
            // The defer above is the only inserter/eraser for
            // cache_key, and the vassert(inserted) right after the
            // emplace rules out duplicate inserts; this guard is
            // purely defensive against future refactors that
