...
private:
    ...
    // Per-reader footer stash. Single entry; replaced on miss. Earns
    // hits when an upstream caller (e.g., the follow-on next-L1
    // prefetch path) has primed the cache with an oid that this
    // reader is about to transition into.
